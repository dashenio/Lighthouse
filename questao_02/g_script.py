...
def limpar_nome(nome: str) -> str:
    ...
    # Função de snake_case
    snake_case = []
    for i, char in enumerate(nome):
        # Se for maiúscula e não for a primeira letra, adiciona _ antes
        if (
            char.isupper()
            and i > 0
            and (nome[i - 1].islower() or nome[i - 1].isdigit())
        ):
            snake_case.append("_")
        snake_case.append(char)

    texto_processado = "".join(snake_case).lower()

    # Mantém letras, números e PRESERVA o underline '_'
    preserva_underline = []
    for char in texto_processado:
        if char.isalnum() or char == "_":
            preserva_underline.append(char)
        elif char in (" ", "-"):
            preserva_underline.append("_")

    # Limpa underlines duplicados e nas pontas
    resultado = "".join(preserva_underline)
...
def tipo_postgres(valor: str) -> str:
    """Infere o tipo de dado do PostgreSQL com base no texto."""
    valor = valor.strip()
...
def csv_para_schema_postgres(
    ...
    for caminho_csv in pasta.glob("*.csv"):
        nome_tabela = limpar_nome(caminho_csv.stem)
        ...
        with open(caminho_csv, mode="r", encoding="utf-8") as f:
            ...
            colunas_sanitizadas = [limpar_nome(col) for col in cabecalho]
            tipos_colunas = {col: "UNKNOWN" for col in colunas_sanitizadas}

            for i, linha in enumerate(leitor):
                if i >= 100:  # Inspeciona as primeiras 100 linhas
                    break

                for col_nome, valor in zip(colunas_sanitizadas, linha):
            ...
            definicoes_colunas = []
            for col_nome in colunas_sanitizadas:
...
# Exemplo de uso:
# csv_para_schema_postgres('./meus_csvs', 'schema.sql')
